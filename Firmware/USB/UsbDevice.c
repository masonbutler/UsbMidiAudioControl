...
static sEndpointDMATransferDescriptor epTransDescr[NUM_EP_DMA_BANKS];

static uint8_t ctrlEndpointOutBuf[64] __attribute__((aligned(4)));
static uint8_t ctrlEndpointInBuf[64] __attribute__((aligned(4)));

static uint8_t midiEndpointInBuf[64] __attribute__((aligned(4)));
...
void UsbSend(sUsbDevice *usbDevice, uint8_t endpointNum, uint8_t *data, uint32_t size)
{
	...
	if(endpointNum == CONTROL_EP_NUM)
	{
		while (bytesLeft)
		{
			...
			module->DEVICE.DeviceEndpoint[CONTROL_EP_NUM].EPINTENSET.bit.TRCPT1 = 1;
...
static void EndpointConfigureControl0(sUsbDevice *usbDevice, UsbDeviceStandardDescriptor *deviceDesc)
{
	...
	assert("Invalid EP0 packet size!");
	...
	// Set in and out EPs ready to send and receive
	...
	// Clear out endpoint ready to receive
	module->DEVICE.DeviceEndpoint[CONTROL_EP_NUM].EPSTATUSCLR.bit.BK0RDY = 1;
...
void UsbHandler()
{
	...
	if (module->DEVICE.DeviceEndpoint[CONTROL_EP_NUM].EPINTFLAG.bit.RXSTP)
	{
		...
		module->DEVICE.DeviceEndpoint[CONTROL_EP_NUM].EPINTFLAG.reg = USB_DEVICE_EPINTFLAG_RXSTP;
		// Clear out endpoint bank ready
		module->DEVICE.DeviceEndpoint[CONTROL_EP_NUM].EPSTATUSCLR.bit.BK0RDY = 1;
	...
	for (int i = 0; endpointInterrupts && i < USB_NUM_ENDPOINTS; i++)
	{
		// Skip endpoint if interrupt flag not set
		if (!(endpointInterrupts & (1U << i)))
		{
			continue;
		}
